...
int main(int argc, char *argv[])
{
    ...
    m1 = paillier_plaintext_from_ui(1);
    paillier_plaintext_t* m2;
    m2 = paillier_plaintext_from_ui(500000);
    ...
    // Do a second sum
    paillier_mul(pubKey, encrypted_sum, ctxt1, encrypted_sum);
    gmp_printf("Sum's ciphertext after second sum: %Zd\n", encrypted_sum);

    // Decrypt the second sum
    dec = paillier_dec(NULL, pubKey, secKey, encrypted_sum);
    gmp_printf("Decrypted: %Zd\n", dec);
